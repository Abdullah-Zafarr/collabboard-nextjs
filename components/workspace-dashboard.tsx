...
import { ArrowRight, LayoutGrid, Plus, Trash2, Users } from "lucide-react";
...
export function WorkspaceDashboard({ userId }: { userId: string }) {
  ...
  async function createWorkspace(event: React.FormEvent) {
    ...
    router.push(`/board/${toWorkspaceSlug(name)}`);
  }

  async function deleteWorkspace(workspace: Workspace) {
    const confirmed = window.confirm(
      `Delete "${workspace.name}"? All tasks, comments, and workspace activity will be permanently removed.`,
    );
    if (!confirmed) return;
    ...
    if (error) setError(error.message);
    else setWorkspaces((current) => current.filter((item) => item.id !== workspace.id));
    setDeletingId(null);
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12 lg:px-8">
      ...
      <form onSubmit={createWorkspace} className="mt-10 flex max-w-2xl gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
        <input
          value={name}
          onChange={(event) => setName(event.target.value)}
          placeholder="New workspace name"
          maxLength={80}
          className="min-w-0 flex-1 rounded-xl px-3 text-sm outline-none placeholder:text-slate-400"
        />
        <button disabled={creating || !name.trim()} className="flex items-center gap-2 rounded-xl bg-emerald-500 px-5 py-3 text-sm font-bold text-white hover:bg-emerald-600 disabled:opacity-50">
          ...
          {workspaces.map((workspace, index) => {
            ...
            return (
              <article key={workspace.id} className="group relative rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-emerald-200 hover:shadow-xl">
                <Link
                  href={`/board/${toWorkspaceSlug(workspace.name)}`}
                  aria-label={`Open ${workspace.name}`}
                  className="absolute inset-0 rounded-3xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
                />
                <div className="pointer-events-none flex items-start justify-between">
                  <span className={`grid size-12 place-items-center rounded-2xl text-white ${["bg-emerald-500", "bg-violet-500", "bg-sky-500"][index % 3]}`}>
                  ...
                  <div className="flex items-center gap-2">
                    {isOwner && (
                      <button
                        ...
                        className="pointer-events-auto relative z-10 grid size-9 place-items-center rounded-xl text-slate-400 transition hover:bg-rose-50 hover:text-rose-600 disabled:opacity-50"
                      >
